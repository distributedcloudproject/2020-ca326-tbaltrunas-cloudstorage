...
func createRequestHandler(node *Node, cloud *Cloud) func(string) interface{} {
	utils.GetLogger().Printf("Creating a request handler for node: %v, cloud: %v.", node, cloud)
	r := request{
		cloud: cloud,
		node: node,
	}
	...
	return func(message string) interface{} {
		utils.GetLogger().Printf("Anonymous handler switcher called for message: %v.", message)
		switch message {
...
func (n *Node) AddNode(node Node) error {
	utils.GetLogger().Printf("Sending AddNode request with parameter node: %v.", node)
	_, err := n.client.SendMessage(AddNodeMsg, node)
	return err
}

func (r request) OnAddNodeRequest(node Node) {
	utils.GetLogger().Printf("Handling AddNodeRequest with parameter node: %v.", node)
	r.cloud.Mutex.Lock()
	...
	for _, n := range r.cloud.Network.Nodes {
		if n.ID == node.ID {
			if n.client == nil {
				utils.GetLogger().Printf("Found matching node with nil client: %v.", n)
				n.IP = node.IP
				n.Name = node.Name
				n.client = node.client
				utils.GetLogger().Printf("Updated matching nil client node: %v.", n)	
			}
			return
		}
	}
	r.cloud.Network.Nodes = append(r.cloud.Network.Nodes, &node)
	utils.GetLogger().Printf("Added node to list of nodes: %v.", r.cloud.Network.Nodes)
	r.cloud.Save()
}